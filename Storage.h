...
private:
	...
	const Date& createCurrDate(); // suzdava data, koyato e tekushtata za kompiutura
public:
	Storage();
	Storage(const Vector<Section>, const Vector<Log>);

	void print();	// izvejda invormaciya za nalichnite producti v sklada
	void add(Vector<Section>&); // dobavya nov product v dialogov rejim
	void remove(Vector<Section>&); // premahva product v dialogov rejim
	void log(const char* , const char*); // izvejda spravka za promenite po nalichnostta na sklada v dadeniya period
